...
def remove_watermark(doc, text_hint: str | None = None) -> int:
    touched = 0
    for i in range(len(doc)):
        ...
        if is_scanned(page):
            ...
            touched += remove_watermark_vector(page, text_hint)
    return touched
...
def parse(text: str):
    ...
    for p in parts:
        ...
        if re.search(r"ווטרמארק|ווטר מארק|סימן מים|watermark", low):
            ...
            ops.append({"type": "watermark", "hint": m.group(1).strip(" \"'") if m else None})
...
def describe(op):
    t = op["type"]
    if t == "watermark":
        return "הסרת ווטרמארק" + (f' ("{op["hint"]}")' if op.get("hint") else "")
    if t == "redact":
...
def apply(pdf_bytes: bytes, ops):
    ...
    for op in ops:
        t = op["type"]
        if t == "watermark":
            n = remove_watermark(doc, op.get("hint"))
